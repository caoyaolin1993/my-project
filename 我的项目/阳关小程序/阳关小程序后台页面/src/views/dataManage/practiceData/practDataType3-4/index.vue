<template>
  <div class="app-container">
    ...
    <el-table v-loading="tab_loading" border :data="tableData" style="width: 100%" max-height="550">
      ...
      <el-table-column prop="type" label="患者分类" header-align="center" align="center"></el-table-column>
      <el-table-column prop="couse_number" label="课程编号" header-align="center" align="center"></el-table-column>
      ...
      <el-table-column prop="situation" label="情境" header-align="center" align="center" width="300"></el-table-column>
      <el-table-column label="情绪" class-name="cust-column" header-align="center" align="center">
        <template slot-scope="scope">
          <div class="cust-cell" v-for="(item,i) in scope.row.info['mood']" :key="i">
            {{item.mood}}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="情绪评分" class-name="cust-column" header-align="center" align="center">
        <template slot-scope="scope">
          <div class="cust-cell" v-for="(item,i) in scope.row.info['mood']" :key="i">
            {{item.fraction || "&nbsp;"}}
          </div>
      ...
      >
        <template slot-scope="scope">
          <div class="cust-cell" v-for="(item,i) in scope.row.info['think']" :key="i">
            {{item.think}}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="思维评分" class-name="cust-column" header-align="center" align="center" width="180px">
        <template slot-scope="scope">
          <div class="cust-cell" v-for="(item,i) in scope.row.info['think']" :key="i">
            {{item.fraction}}
          </div>
      ...
      <el-table-column label="思维误区" class-name="cust-column" header-align="center" align="center" width="180px">
        <template slot-scope="scope">
          <div class="cust-cell" v-for="(item,i) in scope.row.info['think']" :key="i">
...
<script>
import { identify_myth, excel_identify_myth } from "@/api/dataManage/practData";
