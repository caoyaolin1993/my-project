...
class PracticeS6ActivityPlan extends cmPrefix
{
    /**
     * 显示资源列表
     *
    ...
    public function index()
    {
        ...
        $find = ActivityPlanS6::where('open_id', $open_id)->group('stime')->order('stime')->field('stime')->select()->toArray();
    ...
    public function save(Request $request)
    {
        ...
        try {
            Db::transaction(function () use ($info) {
                $activityPlanS6 = app('activityPlanS6');
                $activityPlanS6->saveAll($info);
            });
    ...
    public function read()
    {
        ...
        $res = ActivityPlanS6::where(['open_id' => $open_id, 'stime' => $stime])->field('date,week,activity,step')->select()->toArray();
    ...
    public function update(Request $request)
    {
        ...
        $find = ActivityPlanS6::where(['open_id' => $open_id, 'stime' => $stime])->field('etime,ltime,new,type')->find()->toArray();
        ...
        try {
            Db::transaction(function () use ($info, $open_id, $stime) {
                ActivityPlanS6::destroy(function ($query) use ($open_id, $stime) {
                    $query->where(['open_id' => $open_id, 'stime' => $stime]);
                });
                $activityObj = app('activityPlanS6');
