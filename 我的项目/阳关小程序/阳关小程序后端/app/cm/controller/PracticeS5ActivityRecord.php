...
class PracticeS5ActivityRecord extends cmPrefix
{
    /**
     * 显示资源列表
     *
    ...
    public function index()
    {
        ...
        $res = S5ActivityRecord::where('open_id', $open_id)->field('stime')->order('stime')->group('stime')->select()->toArray();
    ...
    public function save(Request $request)
    {
        ...
        try {
            Db::transaction(function () use ($list) {
                $s5ActivityRecord = app('s5ActivityRecord');
                $s5ActivityRecord->saveAll($list);
            });
    ...
    public function read()
    {
        ...
        $res = S5ActivityRecord::where(['open_id' => $open_id, 'stime' => $stime])->where('activity', '<>', '')->field('date,time,activity,pleasure,achievement')->select()->toArray();
        foreach ($res as $value) {
            $arr[] = [
    ...
    public function update(Request $request)
    {
        ...
        $res = S5ActivityRecord::where(['open_id' => $open_id, 'stime' => $stime])->field('etime,ltime,new,type')->find()->toArray();
        ...
        try {
            Db::transaction(function()use($list,$open_id,$stime){
                S5ActivityRecord::destroy(function($query)use($open_id,$stime){
