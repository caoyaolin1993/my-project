...
class PracticeS5ActivityPlan extends cmPrefix
{
    /**
     * 显示资源列表
     *
    ...
    public function index()
    {
        ...
        $find = ActivityPlanS5::where('open_id', $open_id)->group('stime')->order('stime')->field('stime')->select()->toArray();
    ...
    public function read()
    {
        ...
        $res = ActivityPlanS5::where(['open_id' => $open_id, 'stime' => $stime])->field('date,week,activity,step')->select()->toArray();
    ...
    public function update(Request $request)
    {
        ...
        $find = ActivityPlanS5::where(['open_id' => $open_id, 'stime' => $stime])->field('etime,ltime,new,type')->find()->toArray();
        ...
        try {
            Db::transaction(function () use ($info, $open_id, $stime) {
                ActivityPlanS5::destroy(function ($query) use ($open_id, $stime) {
                    $query->where(['open_id' => $open_id, 'stime' => $stime]);
                });
                $activityObj = app('activityPlanS5');
