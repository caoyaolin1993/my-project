...
class S5Practice extends AdminAuth
{
    //S5归因练习
    public function attributionPractice()
    {
        $where[] = ['a.id', '>', 0];
        ...
        $cachekey = md5($stime . $etime . $number . $name . $phone . implode(',', $type) . $page . $limit . 'admin' . 'S5Practice' . 'attributionPractice');
        if (cache($cachekey)) {
            $return = cache($cachekey);
        } else {
            $list = Db::name('user')->alias('a')->where($where)->join('cm_attribution_practice b', 'a.open_id=b.open_id')->field(['a.open_id', 'a.name', 'a.wx_phone', 'a.type', 'a.number', 'b.things', 'b.main_reason', 'b.stime', 'b.etime', 'b.ltime'])->page($page, $limit)->select()->toArray();

            foreach ($list as $key => $value) {
                $list[$key]['main_reason'] = json_decode($value['main_reason'], true);
                // $arr = [];
                // foreach ($list[$key]['main_reason'] as $k => $v) {
                //     $arr[$v['direction']][] = [
                //         'reason' => $v['reason'],
                //         'proportion' => $v['proportion']
                //     ];
                // }
                // $list[$key]['main_reason'] = $arr;
                ...
                $list[$key]['couse_number'] = 'S5';
                $list[$key]['stime'] = $value['stime'] ? date('Y-m-d H:i', $value['stime']) : '';
                $list[$key]['etime'] = $value['etime'] ? date('Y-m-d H:i', $value['etime']) : '';
            }

            $total =  Db::name('user')->alias('a')->where($where)->join('cm_attribution_practice b', 'a.open_id=b.open_id')->field('a.open_id')->count();
    ...
    //导出S5归因练习
    public function excel_attributionPractice()
    {
        $where[] = ['a.id', '>', 0];
        $stime = input('post.stime');
        $etime = input('post.etime');
        ...
        $PHPSheet->setCellValue('J1', '糟糕事情');
        $PHPSheet->setCellValue('K1', '归因方向');
        $PHPSheet->setCellValue('L1', '具体原因');
        $PHPSheet->setCellValue('M1', '比重');
        ...
        $k = 2;
        foreach ($list as $key => $value) {
            foreach ($value['main_reason1'] as $key1 => $value1) {
                $j = 0;
                $o = count($value1);
                $PHPSheet->setCellValue('K' . $i, $key1)->mergeCells('K' . $i . ':' . 'K' . ($i + $o - 1));
                foreach ($value1 as $key2 => $value2) {
                    $PHPSheet->setCellValue('L' . $i, $value2['reason']);
                    $PHPSheet->setCellValue('M' . $i, $value2['proportion']);
                    $i++;
                    $j++;
                }
                // $PHPSheet->mergeCells('K' . $k . ':' . 'K' . ($k + $j - 1));
            }
            $c = count($value['main_reason']);
            ...
            $PHPSheet->setCellValue('J' . $k, $value['things'])->mergeCells('J' . $k . ':' . 'J' . ($k + $c - 1));
            //自动换行
    ...
    //S5-活动安排
    public function s5_activity_arrange()
    {
        $where[] = ['id', '>', 0];
        ...
        $page = input('post.page', 1);
        $limit = input('post.limit', 10);
        $cachekey = md5($stime . $etime . $number . $name . $phone . implode(',', $type) . $page . $limit . 'admin' . 'S5Practice' . 's5_activity_arrange');
        if (cache($cachekey)) {
            $return = cache($cachekey);
        } else {

            $list = Db::name('activity_plan_s5')->where($where)->group('stime')->field('open_id,stime')->page($page, $limit)->select()->toArray();
            if (empty($list)) {
            ...
            foreach ($list as $key => $value) {
                $actarr = Db::name('activity_plan_s5')->where(['open_id' => $value['open_id'], 'stime' => $value['stime']])->field('open_id,date,week,activity,stime,etime,ltime')->select()->toArray();
                $uearr = [];
    ...
    //导出S5-活动安排
    public function excel_s5_activity_arrange()
    {
        $where[] = ['id', '>', 0];
        ...
        $page = input('post.page', 1);
        $limit = input('post.limit', 10);
        $list = Db::name('activity_plan_s5')->where($where)->group('stime')->page($page, $limit)->field('open_id,stime')->select()->toArray();
        if ($list) {
            foreach ($list as $key => $value) {
                $arrli[] = $value['open_id'];
            }
    ...
    //S5-活动记录
    public function s5_activity_record()
    {
        $where[] = ['a.id', '>', 0];
        $number = input('post.number');
        if ($number) {
        ...
        $page = input('post.page', 1);
        $limit = input('post.limit', 10);
        $cachekey = md5($stime . $etime . $number . $name . $phone . implode(',', $type) . $page . $limit . 'admin' . 'S5Practice' . 's5_activity_record');
        if (cache($cachekey)) {
            $return = cache($cachekey);
        } else {
            ...
            $list = Db::name('user')
                ->alias('a')
                ->where($where)
                ->join('cm_s5_activity_record b', 'a.open_id = b.open_id')
    ...
    //导出S5-活动记录
    public function excel_s5_activity_record()
    {
        $where[] = ['a.id', '>', 0];
        $number = input('post.number');
        if ($number) {
    ...
    public function problemSolving()
    {
        ...
        $page = input('post.page', 1);
        $limit = input('post.limit', 10);
        $cachekey = md5($stime . $etime . $number . $name . $phone . implode(',', $type) . $page . $limit . 'admin' . 'S5Practice' . 'problemSolving');
        if (cache($cachekey)) {
            $return = cache($cachekey);
        } else {
            $list = Db::name('user')
                ->alias('a')
                ->where($where)
                ->join('cm_problem_solving b', 'a.open_id = b.open_id')
                ->field('a.open_id,a.number,a.name,a.wx_phone,a.type,b.stime,b.etime,b.ltime,b.problem,b.target,b.plan,b.selected,b.steps')
                ->page($page, $limit)
                ->order('a.id')
                ->select()->toArray();

            foreach ($list as $key => $value) {
                $list[$key]['plan'] = json_decode($value['plan'], true);
                $list[$key]['steps'] = json_decode($value['steps'], true);
    ...
    //导出S5-问题解决
    public function excel_problemSolving()
    {
        $where[] = ['a.id', '>', 0];
        $number = input('post.number');
        if ($number) {
        ...
        $PHPSheet->setCellValue('H1', '结束时间');
        $PHPSheet->setCellValue('I1', '时长');
        $PHPSheet->setCellValue('J1', '问题');
        $PHPSheet->setCellValue('K1', '目标');
        $PHPSheet->setCellValue('L1', '解决方案');
        $PHPSheet->setCellValue('M1', '优点');
        $PHPSheet->setCellValue('N1', '缺点');
        $PHPSheet->setCellValue('O1', '排序值');
        $PHPSheet->setCellValue('P1', '选定方案');
        $PHPSheet->setCellValue('Q1', '步骤');
        $PHPSheet->setCellValue('R1', '实施时间');

        $PHPSheet->getColumnDimension('A')->setWidth(25);
        $PHPSheet->getColumnDimension('B')->setWidth(20);
        $PHPSheet->getColumnDimension('C')->setWidth(12);
        $PHPSheet->getColumnDimension('D')->setWidth(25);
        $PHPSheet->getColumnDimension('E')->setWidth(25);
        $PHPSheet->getColumnDimension('F')->setWidth(12);
        $PHPSheet->getColumnDimension('G')->setWidth(25);
        $PHPSheet->getColumnDimension('H')->setWidth(25);
        $PHPSheet->getColumnDimension('I')->setWidth(12);
        $PHPSheet->getColumnDimension('J')->setWidth(25);
        $PHPSheet->getColumnDimension('K')->setWidth(25);
        $PHPSheet->getColumnDimension('L')->setWidth(25);
        $PHPSheet->getColumnDimension('M')->setWidth(25);
        $PHPSheet->getColumnDimension('N')->setWidth(25);
        $PHPSheet->getColumnDimension('O')->setWidth(25);
        $PHPSheet->getColumnDimension('P')->setWidth(25);
        $PHPSheet->getColumnDimension('Q')->setWidth(25);
        $PHPSheet->getColumnDimension('R')->setWidth(25);

        $i = 2;
        $j = 2;
        $k = 2;
        foreach ($list as $key => $value) {
            foreach ($value['plan'] as $key1 => $value1) {
                $PHPSheet->setCellValue('L' . $i, $value1['possible_solutions']);
                $PHPSheet->setCellValue('M' . $i, $value1['benefits']);
                $PHPSheet->setCellValue('N' . $i, $value1['bad']);
                $PHPSheet->setCellValue('O' . $i, $value1['number']);
                $i++;
            }

            foreach ($value['steps'] as $key1 => $value1) {
                $PHPSheet->setCellValue('Q' . $j, $value1['steps']);
                $PHPSheet->setCellValue('R' . $j, $value1['time']);
                $j++;
            }

            $m = count($value['plan']) >= count($value['steps']) ? count($value['plan']) : count($value['steps']);

            $PHPSheet->setCellValue('A' . $k, $value['open_id'])->mergeCells('A' . $k . ':' . 'A' . ($k + $m - 1));
            $PHPSheet->setCellValue('B' . $k, $value['number'])->mergeCells('B' . $k . ':' . 'B' . ($k + $m - 1));
            $PHPSheet->setCellValue('C' . $k, $value['name'])->mergeCells('C' . $k . ':' . 'C' . ($k + $m - 1));
            $PHPSheet->setCellValue('D' . $k, $value['wx_phone'])->mergeCells('D' . $k . ':' . 'D' . ($k + $m - 1));
            $PHPSheet->setCellValue('E' . $k, $value['type_name'])->mergeCells('E' . $k . ':' . 'E' . ($k + $m - 1));
            $PHPSheet->setCellValue('F' . $k, $value['couse_number'])->mergeCells('F' . $k . ':' . 'F' . ($k + $m - 1));
            $PHPSheet->setCellValue('G' . $k, $value['stime'])->mergeCells('G' . $k . ':' . 'G' . ($k + $m - 1));
            $PHPSheet->setCellValue('H' . $k, $value['etime'])->mergeCells('H' . $k . ':' . 'H' . ($k + $m - 1));
            $PHPSheet->setCellValue('I' . $k, $value['ltime'])->mergeCells('I' . $k . ':' . 'I' . ($k + $m - 1));
            $PHPSheet->setCellValue('J' . $k, $value['problem'])->mergeCells('J' . $k . ':' . 'J' . ($k + $m - 1));
            $PHPSheet->setCellValue('k' . $k, $value['target'])->mergeCells('k' . $k . ':' . 'k' . ($k + $m - 1));
            $k += $m;
            $i = $j = ($i >= $j) ? $i : $j;
        }

        //设置水平居中
        $PHPSheet->getStyle('A1:R' . ($k - 1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

        //设置垂直居中
        $PHPSheet->getStyle('A1:R' . ($k - 1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);


        $filename = 'S5-问题解决表' . date('Ymd');
        $PHPWriter = PHPExcel_IOFactory::createWriter($PHPExcel, "Excel2007"); //创建生成的格式
        header("Content-Disposition: attachment;filename=$filename.xlsx"); //下载下来的表格名
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        $PHPWriter->save("php://output"); //表示在$path路径下面生成demo.xlsx文件
