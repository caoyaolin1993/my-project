...
class AdminVerifyService
{
    ...
    {
        $key = md5(uniqid((string)mt_rand(10, 99), true));
        $bag = '';
    ...
    public function create(string $config = null, bool $api = false)
    {
        ...
        // 验证码使用随机字体
        $ttfPath = './static/verify/assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        foreach ($text as $index => $char) {
            // $x     = $this->fontSize * ($index + 1) * mt_rand(1, 2) * ($this->math ? 1 : 1.5);
            $x     = $this->fontSize * ($index + 1) * 1.1 * ($this->math ? 1 : 1.5);
