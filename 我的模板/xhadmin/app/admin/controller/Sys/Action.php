...
class Action extends Admin
{
	...
	public function delete(){
		...
		try{
			...
			if($res && $info['type'] == 31){
				...
				if($this->getFieldStatus(config('database.connections.mysql.prefix').$menuInfo['table_name'],$delete_field)){
					$sql = 'ALTER TABLE '.config('database.connections.mysql.prefix').$menuInfo['table_name'].' DROP '.$delete_field;
				Db::execute($sql);
				}	
			}
		} catch (\Exception $e) {
	...
	//删除字段之前 先判断数据表字段是否存在
	public function getFieldStatus($tablename,$field){
