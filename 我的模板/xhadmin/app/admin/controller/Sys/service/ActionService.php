	...
	public static function saveData($type,$data){
		...
		try{
			...
			if($type == 'add'){
				...
				if(!$info){
					...
					if($reset->id){
						...
						if($data['type'] == 31){
							...
							$sql="ALTER TABLE ".config('database.connections.mysql.prefix')."{$menuInfo['table_name']} ADD {$delete_field} int(10) COMMENT '软删除标记' DEFAULT null";
							Db::execute($sql);
						}
					}
				}else{
				...
				if($data['type'] == 31){
					$delete_field = !is_null(config('my.delete_field')) ? config('my.delete_field') : 'delete_time';
					$deleteFieldStatus = self::getFieldStatus(config('database.connections.mysql.prefix').$menuInfo['table_name'],$delete_field);
