					...
					if($val['is_controller_create'] !== 0){
						...
						if($fieldList){
							...
							foreach($fieldList as $k=>$v){
								...
								if(in_array($v['list_show'],[1,2]) && BuildService::getFieldStatus($v['field'],$menuInfo['table_name'])){
				...
				case 15:
					if($val['is_controller_create'] !== 0){
						...
						if(!empty($val['sql_query'])){
							...
							$str .="		\$info = Db::query('".$val['sql_query']." where ".$pre_table."".$pk_id." = '.$".$pk_id.");\n";
							...
							if(!empty($val['relate_table']) && !empty($val['relate_field'])){
								...
								$str .="		\$sql = 'select ".$field." from ".config('database.connections.mysql.prefix').$menuInfo['table_name']." as a left join ".config('database.connections.mysql.prefix').$val['relate_table']." as b on a.".$val['relate_field']." = b.".$val['relate_field']." where a.".$menuInfo['pk_id']." = '.\$".$pk_id.".' limit 1';\n";
					...
					if($val['is_controller_create'] !== 0){
						...
						if(!empty($val['sql_query'])){
							...
							if(!empty($val['relate_table']) && !empty($val['relate_field'])){
								...
								$str .=$s."\$sql = 'select ".$field." from ".config('database.connections.mysql.prefix').$menuInfo['table_name']." as a left join ".config('database.connections.mysql.prefix').$val['relate_table']." as b on a.".$val['relate_field']." = b.".$val['relate_field']." where a.".$menuInfo['pk_id']." = '.\$data['".$pk_id."'].' limit 1';\n";
								$str .=$s."\$res = checkData(current(Db::query(\$sql)));\n";
							}else{
								if(empty($val['fields'])){
	...
	public static function createInfoTpl($applicationInfo,$menuInfo,$actionInfo,$fieldList){
		...
		foreach($fieldList as $key=>$val){
			...
			if($val['type'] == 34 && $val['is_post'] == 1 && in_array($val['tab_menu_name'],$tabList) && in_array($val['field'],explode(',',$actionInfo['fields']))){
				$htmlstr .= "		uploader('".$val['field']."_upload','".$val['field']."','file',true,'','{:url(\"".$applicationInfo['app_dir'].'/Upload/uploadImages'."\")}');\n";
			}
		}
		$htmlstr .= "	});\n";
			...
			foreach($fieldList as $key=>$val){
				if($val['type'] == 8 && $val['is_post'] == 1 && in_array($val['field'],explode(',',$actionInfo['fields']))){
					if($menuInfo['upload_config_id']){
						$htmlstr .= "uploader('".$val['field']."_upload','".$val['field']."','image',false,'','{:url(\"".$applicationInfo['app_dir'].'/Upload/uploadImages'."\",[\"upload_config_id\"=>".$menuInfo['upload_config_id']."])}');\n";
					}else{
						$htmlstr .= "uploader('".$val['field']."_upload','".$val['field']."','image',false,'','{:url(\"".$applicationInfo['app_dir'].'/Upload/uploadImages'."\")}');\n";
					}
				}
				if($val['type'] == 9 && $val['is_post'] == 1 && in_array($val['field'],explode(',',$actionInfo['fields']))){
					if($menuInfo['upload_config_id']){
						$htmlstr .= "uploader('".$val['field']."_upload','".$val['field']."','image',true,'{\$info.".$val['field']."}','{:url(\"".$applicationInfo['app_dir'].'/Upload/uploadImages'."\",[\"upload_config_id\"=>".$menuInfo['upload_config_id']."])}');\n";
					}else{
						$htmlstr .= "uploader('".$val['field']."_upload','".$val['field']."','image',true,'{\$info.".$val['field']."}','{:url(\"".$applicationInfo['app_dir'].'/Upload/uploadImages'."\")}');\n";
					}
				...
				if($val['type'] == 34 && $val['is_post'] == 1 && in_array($val['field'],explode(',',$actionInfo['fields']))){
					$htmlstr .= "uploader('".$val['field']."_upload','".$val['field']."','file',true,'{\$info.".$val['field']."}','{:url(\"".$applicationInfo['app_dir'].'/Upload/uploadImages'."\")}');\n";
				}
			}	
		}
		if($chosen_status){
